...
if Powerlaw==1
    co=(Qm./(RK.*Tm));
    ind=Im>=15;
    co(ind)=((Qm(ind)+Pm(ind).*1.7e-5)./(RK.*Tm(ind)));
    ind=co>150;
    co(ind)=150;
...
%     ind=Im>=15;
%     etam(ind)=1./(1600*10^(-6*3.5)*(ff(ind)./1e6).^1.2);    
...
%     eta_cur     =   2.^((1-nm)./nm).*3.^(-(1+nm)./(2.*nm)).*etam.^(1./nm).*E2ndm.^((1-nm)./nm).*exp(co./nm);    
    ...
    if Diffusion_creep == 1
        co=(Qdm./(RK.*Tm));
        ind=Im>=15;
        co(ind)=((Qdm(ind)+Pm(ind).*0.6e-5)./(RK.*Tm(ind)));
        ind=co>150;
        co(ind)=150;

%         ind=Im>=15;
%         etadm(ind)=1./(2.5e7*(ff(ind)./1e6).^1.0*10^(-6)*10^(-3*6));
